...
- **AC #18.iii / #20 camera-follow on pop-in/pop-out — DEFERRED** per user feedback (2026-04-22, fix commit `863acbd`). First implementation engaged `setFollowTarget` on every pointermove after a pop-out/pop-in threshold crossing, driving the camera at FOLLOW_LERP=0.12 toward a moving drag target. User reported the camera "speeds up dramatically when an item is added or removed from a group" — the single-frame delta at the threshold-crossing was too large for a 12% lerp, producing a visible jerk. Reverted: pop still fires (mutation + pop animation), camera stays put. Grip-phase cluster drag follow (AC #24) is unchanged and feels correct because the handle IS the affordance being dragged. A future polish story can re-enable with a gentler ratio or a cursor-preserving rig.
- The spec's "convert cursor NDC to a target world offset, interpolate camera position toward the implied target" interpretation was MVP-reduced to "lerp camera + controls.target laterally toward `followTarget`" — still live for grip-phase. Replace with a more exact cursor-preserving rig if the feel is off during handle drag.
