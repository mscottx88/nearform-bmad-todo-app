...
- [x] Task 3: Mount `<GlowSource>` inside the LilyPad group + wire per-frame strength (AC: #1-#5, #7)
  ...
  - [x] Pulsing branch: writes `CREATION_PAD_GLOW` + `t · AMBIENT_GLOW_STRENGTH + glow · 0.35` (linear growth + crest boost, matching the rewritten AC #3 — halo grows from 0 and flashes on each wave crest); at pulse-end (`t=1`, `wave=sin(6π)=0`, `decay=0`) the formula naturally lands at `(padHDR, AMBIENT_GLOW_STRENGTH)` for seamless handoff to resting, so no explicit zero-out is needed.
...
- [x] [Review][Patch] FOCUSED_GLOW_STRENGTH comment claims "brighter than ambient" but equals AMBIENT_GLOW_STRENGTH (both 0.22) [LilyPad.tsx:119] — **Applied:** rewrote the comment to document the deliberate `FOCUSED === AMBIENT === 0.22` coupling and the lockstep-tuning note.
- [x] [Review][Patch] totalT shadow-declared inside completing/deleting glow blocks — duplicates outer binding [LilyPad.tsx:~740,~848] — **Applied:** hoisted `totalT` (and `intensity`, for the strength-jump fix below) to the top of each phase branch; body-tint and glow writes now share both values.
- [x] [Review][Patch] JSX comment says "renderOrder={5} on the material" — renderOrder is an Object3D property set on the mesh [LilyPad.tsx:~1228] — **Applied:** rewrote the comment to correctly attribute renderOrder=5 to the glow mesh (Object3D property) and explain the pad(10)/rim(11,12)/glow(5) paint order.
- [x] [Review][Patch] THREE.DoubleSide on GlowSource unnecessary — camera never views disc from below; use FrontSide [GlowSource.tsx:~65] — **Applied:** changed `side={THREE.DoubleSide}` → `side={THREE.FrontSide}` on the shaderMaterial.
- [x] [Review][Patch] WheelEvent dispatched with bubbles:false — change to true for robustness if OrbitControls listener is not on the canvas element directly [ActionPopup.tsx:~101] — **Applied:** changed `bubbles: false` → `bubbles: true` and added a comment explaining why (direct canvas listener still receives it either way; bubbling future-proofs for document/window-scoped listeners).
- [x] [Review][Patch] Completing/deleting first-frame strength jump for completed/error resting pads — AC #1 violation; resting writes strength=0.088 (0.4×ambient) but completing jumps to lerp(0.22,0.6,0)=0.22 on frame 1 [LilyPad.tsx:~730,~838] — **Applied:** lerp source changed from `AMBIENT_GLOW_STRENGTH` to `AMBIENT_GLOW_STRENGTH * intensity`, matching the resting-branch dimming — first completing/deleting frame now lines up exactly with the prior resting frame.
- [x] [Review][Defer] document.querySelector('canvas') brittle with multiple canvases [ActionPopup.tsx:~91] — deferred, pre-existing
- [x] [Review][Defer] focusFlashStartRef ordering coupling between rim block and glow block — maintenance hazard if blocks are refactored [LilyPad.tsx:~948,~1027] — deferred, pre-existing

#### Second-pass adversarial review (2026-04-20)

Findings from Blind Hunter + Edge Case Hunter + Acceptance Auditor layers against commit `f6d6f13`. Items already captured in the first-pass list above are not repeated.

- [x] [Review][Patch] Ripple crest occludes its own halo via depth test [GlowSource.tsx:62-73] — at complete/delete dissolve-start a ripple fires at the pad's position (`LilyPad.tsx:755, 858`); click-ripple amplitudes are 0.45–0.7 world units (`WaterSurface.tsx:39-40`) while the glow disc sits at world y≈0.01. The glow material has `depthWrite:false` but default `depthTest:true`, so halo pixels under the wave crest fail the depth test and get discarded precisely at peak brightness. **Applied:** added `depthTest={false}` on the `<shaderMaterial>` in `GlowSource.tsx` (standard for additive-blend bloom sources).
- [x] [Review][Patch] Remount mid-sequence: halo invisible for ~0.4s as color lerps from (0,0,0) [LilyPad.tsx:740-748, 838-848 / GlowSource.tsx:54-58] — `GlowSource` initializes `uColor` to `Vector3(0,0,0)` on mount; on LilyPad remount with a persisted `startedAt` (HMR, parent re-key, error-boundary retry) the completing/deleting branch immediately does `uColor.lerp(COMPLETE_PAD_TINT, 0.06)` from black. Since `gl_FragColor = uColor * uStrength * falloff`, additive contribution is ~zero for the first ~25 frames — the halo is invisible while the pad is mid-sequence. Violates AC #1 "no frame where the halo visibly disappears" on remount. **Applied:** seed `uColor` with the pad's HDR base color at the `resting → completing` and `resting → deleting` transition ticks before the per-frame lerp runs.
- [x] [Review][Patch] Focused-branch strength ignores completion/decay `intensity` [LilyPad.tsx:998-1022] — the `if (focused)` branch sets `strength = FOCUSED_GLOW_STRENGTH` (0.22) unmodified while the `else` branch multiplies by `intensity = errorEntry ? DECAY_SATURATION : todo.completed ? 0.4 : 1.0`. A focused completed or errored pad glows at full ambient strength while the pad body is dimmed — halo outshines its source. **Applied:** hoisted `intensity` out of the branches and multiplied it into the focused branch's strength write.
- [x] [Review][Patch] Stale `focusFlashStartRef` survives cancel-recovery → spurious flash replay [LilyPad.tsx:583-601 deleting recovery, 633-647 completing recovery] — both cancel-recovery paths clear `focusFlashPendingRef.current` but not `focusFlashStartRef.current`. If the user clicks a pad (flash stamp written), clicks Complete/Delete within <0.4s, then an external actor cancels the sequence, the glow block's `if (focusFlashStartRef.current !== null)` at line 1027 still sees a fresh stamp on return to `resting` and replays the flash overlay. **Applied:** added `focusFlashStartRef.current = null;` alongside the existing `focusFlashPendingRef.current = false;` in both recovery blocks.
- [x] [Review][Patch] Focused oscillation phase uses raw wall-clock [LilyPad.tsx:999-1002] — `osc = 0.5 + 0.5 * sin(state.clock.elapsedTime * 2π / FOCUSED_OSC_PERIOD_S)` has no anchor to focus-start time, so the starting phase when the popup opens is arbitrary (anywhere in [0,1]). Sometimes the focused halo pops open already near-white and breathes back to pad-color; sometimes it opens near pad-color. Inconsistent "wake up" feel. **Applied:** added a `focusStartTimeRef` stamped lazily on first focused-tick in useFrame and reset in the `focused` useEffect on true→false transition; `osc` now reads `(elapsedTime - focusStartTimeRef.current)`.
- [x] [Review][Patch] Spec-internal contradiction — Task-3 bullet lists the pre-rewrite creation-pulse formula [2-8-pad-action-glow-on-water.md:57] — bullet says creation-pulse writes `max(0, wave) · decay · PAD_TINT_MAX`, but AC #3 (rewritten 2026-04-17) specifies `t · AMBIENT_GLOW_STRENGTH + glow · 0.35` and the code at `LilyPad.tsx:1137-1139` matches AC #3. **Applied:** rewrote the Task-3 bullet to match the AC and added the `sin(6π)=0, decay=0` math-note so future readers see why the formula lands at AMBIENT without an explicit zero-out.
- [x] [Review][Defer] Pulse→resting strength handoff depends on numeric coincidence `AMBIENT_GLOW_STRENGTH === FOCUSED_GLOW_STRENGTH === 0.22` [LilyPad.tsx:119,125,1137-1139] — pulse ends at strength 0.22 (linear term at t=1 + zero crest since `wave = sin(6π) = 0` and `decay = 0`) and the next frame's resting write also lands at 0.22 regardless of `focused`. Seamless today only because both constants equal 0.22; if either is tuned independently a step-change appears at pulse-end. Deferred — no user impact as shipped; logged so future tuning doesn't accidentally desync.

**Review layer summary:** 6 patch findings, 1 new defer (plus 2 pre-existing defers already above). Dismissed as noise: 9 findings (rim-lerp-during-flash false positive — lerp is guarded by `focusFlashStartRef === null`; focused+flash double-lerp — peak still snaps to `FOCUS_PAD_GLOW` since `lerp(a,b,1)=b`; renderOrder semantics misread; alpha=1.0 is spec-required per AC #6; pulse-end math is exact — `sin(6π)·(1-1)=0`; intensity-on-color vs intensity-on-strength matches AC #9; fade-pad-materials touching glow material is spec-consistent since AC #7 requires terminal strength snap; focusFlashStartRef post-completion residue has no behavioral impact; first-frame strength step for completed pads already captured in first-pass findings).
