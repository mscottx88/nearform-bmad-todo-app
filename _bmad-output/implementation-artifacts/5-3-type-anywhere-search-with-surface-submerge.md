...
4. **Given** a LilyPad is a **match**, **When** `useFrame` runs, **Then** the pad's **glow strength** scales by `sqrt(score)` where `score` is the match's score from the API response. A strong hit (score ~0.95) glows at ~97% of full; a borderline hit (score ~0.30) glows at ~55%; a non-match glows at 0. The `sqrt` remap biases the low-middle of the score range upward so moderate matches stay visibly lit rather than reading as "barely on". The pad's body colour, rim colour, Y position, and opacity are **unchanged** from the no-search baseline — the halo is the only search-driven visual on the pad.

5. **Given** a LilyPad is a **non-match**, **When** `useFrame` runs, **Then** the pad's glow strength lerps to 0 (halo snuffed) and every other visual attribute — body colour, rim, Y position, opacity, ambient animation — stays at its no-search baseline. A non-match pad reads as "dormant" purely through the absence of halo. Pads mid-completion/deletion phases are untouched by the search glow override (automatic via the `resting`-phase scope) — their existing colour/dissolve choreography is authoritative.
...
7. **Given** a search is running, **When** results arrive, **Then** the camera is **NOT** moved — no pan, no zoom, no orbit change. Every pad stays at its existing screen position so the user never loses their spatial context. `cameraFocus` is neither set nor cleared by the search path. This supersedes the earlier design (which auto-framed the match centroid and adjusted zoom); per user feedback the camera shift made pads slide off-screen and was disorienting. The only search-driven visual on pads is the glow scaling in AC #4 / #5.
...
12. **Given** the user presses **Escape**, **When** the handler runs, **Then** (a) `searchQuery` clears to `""`, (b) `searchResults` clears to an empty Map, (c) `cameraFocus` is **not** touched — search never moved it in the first place (AC #7), so there's nothing to reset, (d) all LilyPads lerp their glow back to the ambient / focused baseline within the existing lerp speed. The full glow restore completes within **~400 ms** per the UX timeline. The search text dissolves from the water via CSS opacity transition (not an abrupt unmount) — use `transition: opacity 200ms ease-out` on the overlay.

13. **Given** the existing vitest suite + this story's new tests, **When** the suite runs, **Then** (a) all existing tests still pass (no regressions in `PondScene.test.tsx`, `TodoInput.test.tsx`, `useClosePopupOnEscape.test.ts`, `usePopupComplete.test.ts`, `usePopupDelete.test.ts`, popup-color-swatch tests, etc.), AND (b) new tests cover: debounce behaviour (type 3 chars rapidly → only the last triggers a query after 300 ms), Escape-clears-on-searchActive, ignore-keydown-when-input-focused, match-vs-non-match classification in the store, the **search-never-touches-`cameraFocus` invariant** (both directions: running a search doesn't set `cameraFocus`; clearing a search doesn't reset a sentinel `cameraFocus`), empty-result-all-submerge behaviour, `vectorSearchUnavailable` badge render, `ftsSupported=false` treats-all-as-match behaviour.
...
- [x] Task 9: Unit tests — `frontend/src/hooks/usePondSearchSync.test.ts` (AC: #2, #3, #7, #8, #9, #10)
  ...
  - [x] **Search-never-touches-`cameraFocus` invariant**: with a sentinel `cameraFocus` set in the store by some other source (popup open, etc.), running a search + applying results leaves `cameraFocus` equal to the sentinel — neither set nor cleared by the search path.
  - [x] Converse direction: with `cameraFocus` null, running a search and receiving matches leaves `cameraFocus` null — no centroid math, no `focusCamera` dispatch.
...
|---|---|---|---|---|
| 1 | [useKeyboardShortcuts.ts:28](frontend/src/hooks/useKeyboardShortcuts.ts#L28) | window | `Enter` (post-Task 2b rebind, guarded by `!activePopupTodoId && !searchActive`) | No collision after rebind. Previously claimed bare `n`/`N`/`/` which collided with type-anywhere search; Task 2b retired those bindings. |
...
Code review run 2026-04-21. Three parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) across the net `1687d01..HEAD` diff of Story 5.3. 8 patches, 7 deferred, 10+ dismissed as noise/false-positive/duplicate.

**Patches (fix now):**

- [x] [Review][Patch] Stale search state leaks after clear + next search. `clearSearch` resets `searchQuery` but `usePondSearchSync`'s local `debouncedQuery` state never resets, and React Query's 30 s `staleTime` returns the prior query's cached response before the new debounce fires. Also: `applySearchResponse` has no guard on `searchActive`, so an in-flight response landing after Escape repopulates `searchResults`/`vectorSearchUnavailable`. Fix: (a) reset `debouncedQuery` to `''` when `searchActive` flips false via a watching `useEffect`, (b) gate `applySearchResponse` on `usePondStore.getState().searchActive` at the top so a late response to a cleared search is a no-op. [frontend/src/hooks/usePondSearchSync.ts:30, 45]
- [x] [Review][Patch] `backspaceSearch` leaves `searchResults`/`searchAllMatches`/`vectorSearchUnavailable` stale when the query empties. Asymmetric with `clearSearch` (which clears all five). Latent today because LilyPad gates on `searchActive`, but a future consumer reading `searchResults` without that gate would see ghost hits; the pad visuals also briefly display the old matches if a new query fires before the new response lands. Fix: when `backspaceSearch` produces empty `searchQuery`, also reset the four derived search fields. [frontend/src/stores/usePondStore.ts:389-394]
- [x] [Review][Patch] Whitespace-only query activates search but fires no request. Pressing Space when no input is focused sets `searchQuery = ' '`, `searchActive = true`, fades the overlay in with an invisible glyph, but `useSearch`'s `enabled && query.trim().length > 0` gate blocks the backend call. Meanwhile LilyPad treats every pad as non-match (empty `searchResults`) and halos snuff. Fix: in `appendSearchChar`, skip the write if the new composed query is whitespace-only (OR: keep the char but don't flip `searchActive` until `trim().length > 0`). [frontend/src/stores/usePondStore.ts:383-388, frontend/src/hooks/usePondSearchKeyboard.ts:61-64]
- [x] [Review][Patch] `require_test_database` substring heuristic admits production-looking names. `"test" in "latest"`, `"greatest"`, `"contest"`, `"latest_backup"`, `"attestations"` all return true and bypass the `_clean_db` wipe-safeguard. A developer with a real DB named `latest_backup` or `greatest_hits` would silently lose data. Also: a URL like `postgresql://host/test?dbname=prod_data` extracts path `"test"` as the db name and passes, while actually connecting to `prod_data` via the query param. Fix: tighten the match to require the name to END with `_test`, BE exactly `test`, or START with `test_` — substring match is too loose for a safety guard. [backend/tests/_safeguard.py:27-28]
- [x] [Review][Patch] `!data.ftsSupported` matches `undefined`, sending every live pad to the "all match" path on schema drift. If the backend ever returns a partial payload (or the interceptor truncates), every pad lights up instead of failing closed. Fix: explicit `data.ftsSupported === false` comparison. [frontend/src/hooks/usePondSearchSync.ts:54]
- [x] [Review][Patch] `Math.sqrt(searchHit.score)` has no defense against negative or NaN scores. Comment says the backend's `Field(ge=0, le=1)` makes this safe, but the codebase's prevailing pattern is defense-in-depth on GPU writes (`NaN` propagates through `THREE.MathUtils.lerp` into the uniform → black pad on many drivers). Fix: `Math.sqrt(Math.max(0, Number.isFinite(searchHit.score) ? searchHit.score : 0))`. [frontend/src/components/pond/LilyPad.tsx:1105]
- [x] [Review][Patch] No frontend max-length guard before the backend's 500-char `Query` limit. A stuck key in key-repeat mode (~30 keys/sec) crosses 500 chars and each subsequent keystroke triggers a 422 from the backend; the default React Query retry policy (3 retries + backoff) then multiplies those errors. Fix: in `appendSearchChar`, cap at the same 500 the backend enforces. [frontend/src/stores/usePondStore.ts:383-388]
- [x] [Review][Patch] Spec text drift on AC #4 / AC #5 / AC #13 / Task 9 / Dev Notes keyboard audit. The AC text still describes the reverted-away `SEARCH_NEUTRAL_GRAY` desaturation design and the removed camera-framing centroid tests. A strict reader of the current spec would flag the implementation as incomplete even though it matches the final (glow-only, no camera movement) design documented in the Change Log. Fix: rewrite AC #4 + AC #5 to describe the glow-only scaling; update AC #13's test bullet and Task 9's sub-bullets to match the `never-touches-cameraFocus` invariant; update the keyboard-handler audit table to reference the post-rebind line numbers. [Spec lines 30-34, 48, 160-163, 186-187]

**Deferred (real, not this story's scope or signal):**

- [x] [Review][Defer] IME / non-Latin composition input is dropped by the `e.key.length === 1` filter. Japanese/Chinese/Korean commit events arrive with multi-char strings; emoji arrives as surrogate-pair length-2 strings; assistive-tech synthesised events often have `"Unidentified"` as the key name. None reach the search. Dev Notes § "Why no search-`<input>` element" already flags this as a v1 English-UI acknowledged limitation. Add a `compositionstart`/`compositionend`/`input` fallback when i18n becomes real.
- [x] [Review][Defer] `usePondSearchKeyboard` double-mount would double-append keystrokes. No code path mounts the hook twice today, but there's no guard. HMR double-render during dev sometimes produces doubled listeners briefly. Cheap fix: single module-scope listener-registered boolean, OR dedupe via a ref.
- [x] [Review][Defer] In-flight axios request is not aborted on `clearSearch`. Mostly annoying resource waste once the P1 patch gates `applySearchResponse` on `searchActive`. A proper fix threads an `AbortController` through `searchApi.ts`. Defer until request-cancel tests are written.
- [x] [Review][Defer] `selectSearchHit(id)` creates a fresh closure per render AND `setSearchResults` builds fresh `SearchHit` objects per response, so every pad re-renders on every response even though only a handful actually changed match status. O(N) re-renders per debounced keystroke. Optimise with `useShallow` or an externally-cached hit object when pad count crosses ~50 or perf profiling flags it.
- [x] [Review][Defer] Search captures keystrokes when TodoInput is open but focus has drifted to body (clicking outside the input). `e.target.tagName === 'INPUT'` only catches the in-focus case. Edge behaviour — user thinks they're typing into the TodoInput but text lands on the search overlay. Fix: also check `document.activeElement` for the same tagName.
- [x] [Review][Defer] No LilyPad `useFrame` search-mode unit test — the phase guard ("skip search during `completing`/`deleting`") is only structural (the code lives inside the `resting` branch by happenstance, not by explicit guard). A future refactor moving the block could silently regress it. Testing Standards section forbids full-canvas R3F tests in jsdom, so this is an acknowledged gap; add when the Test Architect module ships canvas mocks.
- [x] [Review][Defer] `_fts_supported` DB call has no try/except fallback. Transient Postgres hiccup (tsquery locale config drift, missing extension, etc.) propagates out and kills the whole search instead of letting the embedding/vector branch carry on. Wrap in `try/except` and treat "couldn't run the tsquery" as equivalent to `fts_supported=False`.

**Dismissed (false positive / covered / in-spec / out-of-scope):**

- Dismissed: Backspace app-wide hijack — app is a single-page pond; Backspace-as-history is non-standard on such pages, the preventDefault is intentional.
- Dismissed: `applySearchResponse` reference-equality non-refire — subsumed by the P1 gate fix.
- Dismissed: Runaway debounce on held-down key — self-resolving on key release.
- Dismissed: `vectorSearchUnavailable` persists during the next response's debounce window — self-correcting in ~300ms.
- Dismissed: Saturation ref only updates during `resting` phase — by design; pads in forming/completing/deleting have authoritative visuals.
- Dismissed: Safeguard session-scope vs `_clean_db` function-scope ordering — pytest fixture ordering is stable; non-issue.
- Dismissed: Missing DATABASE_URL env silently uses config default — the default is `todo_pond` (no "test"), so the safeguard catches it; coincidental but fine.
- Dismissed: Popup close + search-active two-Escape dance — documented as cascade semantics in the Keyboard-handler audit; no spec violation.
- Dismissed: `+inf` similarity passes the 0.60 floor but is caught by the later `math.isfinite` — downstream filter catches it; no user impact.
- Dismissed: Enter mid-response opening new-todo with in-flight stale response — edge race; visual flicker only.
- Dismissed: AC #13/Task 9 camera-auto-frame test bullets (folded into the spec-drift P8 patch).
- Dismissed: Dev Notes § subscribing-to-slices paragraph — equivalent-or-better implementation; not a code issue.
...
| 2026-04-21 | UX redesign (again): reverted pad-body + rim desaturation entirely; search visual is now PURELY glow-strength scaling on the halo beneath each pad. Matches glow at `sqrt(score)` strength; non-matches have no halo; pads themselves look identical to non-search. Removed shader `uSaturation` / `uSearchActive` uniforms + mix block; removed body-uColor gray-lerp; removed rim gray-lerp; removed `SEARCH_NEUTRAL_GRAY` constant. Simpler, less visually aggressive. |
| 2026-04-21 | AC #7 revised + auto-frame removed: search no longer moves the camera. Per user directive ("when searching, all pads should be visible, don't adjust the zoom or move the camera around"). Stripped the centroid + bbox-zoom math + `focusCamera` dispatch from `usePondSearchSync.ts`, removed `cameraFocus: null` from `clearSearch`, replaced the two camera-framing tests with a single "search never touches cameraFocus" invariant that pins the new contract in both directions. AC #12 updated to match. 141/141 vitest, tsc clean. |
| 2026-04-21 | Code-review pass: all 8 patches applied. (1) `usePondSearchSync`: late responses gated on `searchActive`; debounce slot flushed to `''` when search goes inactive; explicit `ftsSupported === false` check (no fail-open on schema drift). (2) `backspaceSearch` resets the four derived search fields when the query empties (symmetric with `clearSearch`). (3) `appendSearchChar` skips writes that would leave the query whitespace-only (prevents "active but no request" limbo). (4) `appendSearchChar` caps at new `SEARCH_MAX_LENGTH = 500` to mirror the backend `Query` validator — protects against key-repeat flooding the backend with 422s + React Query retry amplification. (5) `require_test_database` tightened from substring `"test" in name` to exact `test` / `test_*` / `*_test` — the old heuristic admitted `latest_backup`, `greatest_hits`, `contest`, etc. (6) `LilyPad` `Math.sqrt(score)` gains a `Number.isFinite` + `Math.max(0, …)` guard to stop NaN / negative scores from propagating into shader uniforms. (7) Spec drift: AC #4 / #5 rewritten to describe the glow-only scaling (replacing the reverted-away `SEARCH_NEUTRAL_GRAY` desaturation prose); AC #13 test-bullet + Task 9 sub-bullets rewritten around the `search-never-touches-cameraFocus` invariant (replacing the removed camera-auto-frame centroid tests); keyboard-handler audit row #1 updated to reference the post-rebind line 28 in `useKeyboardShortcuts.ts`. Gates: 141/141 vitest, tsc clean, 92/92 pytest, mypy strict clean, ruff clean. |
