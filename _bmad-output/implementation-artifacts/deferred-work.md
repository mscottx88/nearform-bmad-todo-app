...
- `[FIXED]` `mouseNDC` NDC computation in `handlePointerUp` uses `window.innerWidth / window.innerHeight` instead of `gl.domElement.getBoundingClientRect()`. If the canvas ever doesn't fill the entire viewport (sidebar, toolbar, inset), the raycaster ray will miss the clicked point and water-plane intersection will be wrong. Pre-existing since Story 1.x. Fix applied (2026-04-22, 3.1 CR cycle): `const rect = gl.domElement.getBoundingClientRect(); mouseNDC.set((e.clientX - rect.left) / rect.width * 2 - 1, -(e.clientY - rect.top) / rect.height * 2 + 1)`. Test stub added to `PondCamera.test.tsx` so `mockCanvas.getBoundingClientRect()` returns a 1024×768 rect (JSDOM would otherwise return zeros). SHA to be filled at commit. (frontend/src/components/pond/PondCamera.tsx — handlePointerUp)
