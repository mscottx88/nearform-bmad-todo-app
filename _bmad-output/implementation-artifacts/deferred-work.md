...
## Deferred from: code review of 1-5-deployment-and-e2e-readiness (2026-04-27)

- [OPEN] **`force: true` clicks bypass click-through-overlay check** [frontend/e2e/complete-todo.spec.ts, frontend/e2e/delete-todo.spec.ts] — comments say it's because `<Html />` updates the wrapper transform every frame, but `force: true` also bypasses pointer-events visibility. A future overlay regression silently passes. Fix: dispatchEvent('click') or query the underlying button via JS and call .click() instead.
- [OPEN] **`waitForTimeout(200/300/50)` flake antipattern** [frontend/e2e/{search,agent-panel-resize,a11y}.spec.ts] — hard-coded sleeps mask the keyboard-hook mount race. Proper fix: hooks set a `data-ready` attribute on attach, tests `waitForSelector('[data-ready]')`.
- [OPEN] **nginx + frontend Dockerfile lack `HEALTHCHECK` directives** — failed-to-start nginx surfaces as a cryptic 502 instead of container-level red. Add `HEALTHCHECK CMD wget -q -O- http://localhost/ || exit 1` to frontend/Dockerfile.
- [OPEN] **Multi-replica `alembic upgrade head` race** [backend/entrypoint.sh] — every container start runs migrations; with N>1 replicas they race. First wins, others IntegrityError. Needs `pg_advisory_lock` serialization in `migrations/env.py` before multi-replica deploys.
- [OPEN] **`entrypoint.sh` conflates migration failure and DB-not-ready** [backend/entrypoint.sh] — `set -eu` + alembic with no retry. Compose path uses `condition: service_healthy` so the dev case is OK; standalone `docker run` (per the Dockerfile's documented invocation) hits cold-DB races on WSL2/Docker Desktop.
- [OPEN] **Search spec may race against StrictMode mountCount sentinel under HMR** [frontend/src/hooks/usePondSearchKeyboard.ts] — `mountCount` module-scoped guard early-returns the second mount. Under StrictMode + Vite HMR, the second mount could land before the first's effect cleanup, leaving no listener registered. The 200ms breathing room handles observed cases; a fundamental fix is a `data-ready` attribute.
- [OPEN] **Complete/Delete tests assume popup buttons render after fetch lands** [frontend/e2e/{complete,delete}-todo.spec.ts] — relies on `useTodos()` query resolving before the popup-render path runs. 5s default `toBeVisible` timeout passes today; fragile on slow CI cold-starts. Consider explicit wait-for-todos-loaded helper.
- [OPEN] **axe `disableRules` is coarser than per-instance allowlist** [frontend/e2e/a11y.spec.ts] — current allowlist is empty so no live divergence. Spec's "each entry must carry a comment + tracking link" implies finer-grained allowance. Refactor to per-violation suppression when the first entry needs to land.
- [OPEN] **uv image pinned by tag, not digest** [backend/Dockerfile:592] — `ghcr.io/astral-sh/uv:0.5.11` is a floating tag. Supply-chain hardening for production. Pin to `@sha256:...` if/when this image goes to prod.
- [OPEN] **Frontend image 99.2 MB vs ~80 MB AC target** — disclosed in DAR; +19 MB on `nginx:alpine` + 1.4 MB SPA bundle. Marginal optimization possible (custom slim nginx or distroless static-server) but story's "~80 MB" rounding tolerates it.
- [OPEN] **D5: backend image weighs 1.36 GB vs AC 1's ~300 MB target** [backend/Dockerfile] — disclosed in story DAR. Bulk = CrewAI vector-store transitives (pyarrow 153 MB + lancedb 118 MB + onnxruntime 55 MB + chromadb_rust_bindings 51 MB ≈ 377 MB) we don't use (we use pgvector directly). Resolution requires either upstream CrewAI packaging changes (drop the default vector-store extras tree) or amending AC 1 to "~1.5 GB while CrewAI's vector deps remain". Both materially change story outcome — out of scope for this code review pass. Promote to a follow-up story if image-size hardening becomes a deploy blocker.
