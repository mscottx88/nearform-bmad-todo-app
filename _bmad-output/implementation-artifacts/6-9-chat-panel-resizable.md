...
| 2026-04-27 | Three follow-up fixes (user feedback): (1) firefly cursor froze during drag — root cause was `setPointerCapture` suppressing compatibility mousemove events on `window`; CursorFirefly now also listens to `pointermove`. (2) increased default panel width from 440 → 520 (`AGENT_PANEL_DEFAULT_WIDTH` + CSS fallback). (3) added a persistent grip-dot affordance on the handle (3 vertical neon-cyan dots via `::before` + `box-shadow` stack) — dim at rest, full-strength on hover/focus/active. (4) `useGlobalCursorMode` was clobbering the `'resize-h'` mode back to `'firefly'` on every mousemove via its DOM-walk inference; added `'resize-h'` to its skip-override list (mirrors the existing `'grabbing'` carve-out). 599/599 still pass; tsc + lint clean. |
