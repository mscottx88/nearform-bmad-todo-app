...
2. **Given** the `InfoPopup` is visible in hover-only mode, **When** I move the cursor off the pad (and NOT onto the popup itself in this mode), **Then** the popup dismisses: the panel runs a ~120 ms `info-popup-exit` animation — `clip-path` contracts from `inset(0 0 0 0)` back to the central point `inset(50%)`, accompanied by `opacity 1 → 0` — and `PondScene` unmounts it once the animation completes. Hover-mode does not require the cursor to stay on the pad for any "grace period" — it tracks `onPointerLeave` directly.

2a. **Given** a drag of THIS pad starts (crosses the 4 px click/drag threshold), **Then** the pad immediately clears `hoveredTodoId` if it owned the hover, so the info popup dismisses via the same 120 ms exit window as AC #2. The popup does NOT follow the pad during the drag. (CR reversal 2026-04-23 — see Dev Agent Record IN #1.)

2b. **Given** any visibility state, **When** the info popup first appears, **Then** its panel plays a 200 ms grow-from-center-point animation (`info-popup-grow` keyframe: `clip-path` expands from `inset(50% 50% 50% 50%)` — the whole panel clipped to a single central point — out to `inset(0 0 0 0)` over 200 ms with `cubic-bezier(0.16, 1, 0.3, 1)`). The panel's direct children stay at `opacity: 0` during the grow and then fade in over 120 ms via `info-popup-content-reveal` with a 200 ms delay, so the *contents* only appear once the box has reached full size. (CR amendment 2026-04-24 — replaces the earlier slide + fade-in design per user request.)

2c. **Given** the info popup is visible in focused mode (`pointer-events: auto` on the panel), **When** I move the cursor onto the panel body, **Then** the custom cursor reverts from `'grab'` to `'firefly'`. R3F's pointer-event state may still consider the pad underneath "hovered" (since the canvas stops receiving `pointermove` once the panel absorbs events), leaving the frog-hand glyph stuck on the popup — the panel's `onMouseEnter` / `onMouseLeave` handlers explicitly manage the cursor mode to fix this. Only draggable children within the popup (scrollbar thumb, resize handle) show the `'grab'` glyph; the panel body itself shows `'firefly'`.
...
13. **Given** the popup's callout SVG, **Then** it draws a neon-cyan line from the **pad centroid** (SVG origin at the `.info-popup` zero-size wrapper) to the **popup centroid** (panel's geometric center, computed from a live `ResizeObserver` measurement so the endpoint stays correct as the panel grows / shrinks — hover → focused adds action buttons, edit mode adds a user-resizable editor). Line styling: `stroke: var(--neon-cyan)`, `stroke-width: 1.5`, `filter: drop-shadow(0 0 4px var(--neon-cyan))`. Z-order: the SVG is a DOM sibling of the panel, declared BEFORE it, so it paints behind the panel but above the lily pad (inside the drei `<Html>` portal, which itself renders above the canvas). (CR amendment 2026-04-24 — earlier version drew pad anchor → panel top-left corner.)
